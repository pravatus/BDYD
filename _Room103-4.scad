...
module room103_rail1() {
    path = turtle3d([
        "move",wall_thickness 
        ,"arcup",wall_thickness/2,135 //handle
        ,"move", wall_thickness*3-handle_width // center
        ,"arcup",wall_thickness/2,45
    ...
    translate([-room103_walkway_width-wall_thickness,wall_thickness*2,1.5-wall_thickness*2]) // 0 on the stair edge includes 0.6+0.9m rail h
    translate([0,(-door_leaf_width-door_frame_width*2)/2,0]) // clear door
    translate([0,-wall_thickness,0]) // clear mud room
    color("blue") translate([wall_thickness/2,-stair_run*5,0]) // clear stairs
        rotate([0,0,90])
...
module room103_rail2() {
    ...
    translate([wall_size_outer/2,-wall_size_inner/8,0]) // where our door is at 
    translate([-room103_walkway_width-wall_thickness,0,1.5-wall_thickness*2]) // 0 on the stair edge includes 0.6+0.9m rail h
    translate([0,(+door_leaf_width+door_frame_width*2)/2,0]) // clear door
    translate([0,+wall_thickness,0]) // clear mud room
    translate([wall_thickness/2,+stair_run*5-wall_thickness*2,0]) // clear stairs-ish
        rotate([0,0,270]) 
            color("blue") sweep(rect([handle_width/2,handle_width],chamfer=default_chamfer),path2);
}
...
module room103_assembly() {
    ...
    // SLOW
    translate([-15,0,0.8 - mesh_fix]) rotate([0,0,-90]) firma4();
    couch_base_geom();
    outer_wall_extras();
    room103_rail1();
    room103_rail2();
